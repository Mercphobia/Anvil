...
import dev.anvil.ade.system.XmlResourceInterceptor
...
class MockupTools(private val workspaceRoot: File) {
    ...
    suspend fun previewMockup(xmlContent: String): String = withContext(Dispatchers.IO) {
        try {
            ...
            dev.anvil.ade.ui.screens.MockupState.currentXml.value = xmlContent
            dev.anvil.ade.ui.screens.MockupState.lastValidation.value = "XML OK"
    ...
    private fun writeSafe(path: String, content: String): String {
        return try {
            ...
            if (file.exists()) {
                val backup = File(file.parentFile, file.name + ".anvil.bak")
